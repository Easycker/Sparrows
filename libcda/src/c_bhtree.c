...
BHTREE_NODE bhtree_Remove(BHTREE_NODE in_cur,C_BHTREE *tag_tree)
{
	...
	else/*有两个孩子时找出其后继*/
	{
		...
		{
			for(op=node_right(in_cur);node_left(op)!=NULL;op=node_left(op));
		};
	...
	*/
	...
	pool_Free(bhtree_Head(op),&tag_tree->data_pool);
...
void bhtree_Drop(C_BHTREE *tag_tree)
{
	...
	if(tag_tree->tree_root!=NULL&&tag_tree->free_fun!=NULL)
	{
		...
		while(stack_Head(stack_op)->array_length>0)
		{
			...
			if(node_left(op)!=NULL)stack_Append(&stack_op,&node_left(op));
			if(node_right(op)!=NULL)stack_Append(&stack_op,&node_right(op));
