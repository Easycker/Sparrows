...
PRTREE_NODE prtree_Append(void *in_data,C_PRTREE *tag_tree)
{
	...
	if(tree_op!=NULL)/*tree already exits*/
	{
		if(tag_tree->compare_fun(in_data,tree_op))
		{
			...
			MEMCPY_(new_data,in_data,tag_tree->data_size);
		}
		else
		{
			new_data=pool_Malloc(&tag_tree->data_pool);
...
PRTREE_NODE prtree_Remove(PRTREE_NODE in_cur,C_PRTREE *tag_tree)
{
	...
	else/*有两个孩子时找出其后继*/
	{
		...
		{
			op=in_cur;
			for(op=prnode_right(in_cur);prnode_left(op)!=NULL;op=prnode_left(op));
			memcpy(in_cur,op,tag_tree->data_size);
		}
	};
	...
	*/
	...
	if(tag_tree->free_fun!=NULL)tag_tree->free_fun(*(void**)op);
	pool_Free(prtree_Head(op),&tag_tree->data_pool);
...
void prtree_Drop(C_PRTREE *tag_tree)
{
	...
	if(tag_tree->tree_root!=NULL&&tag_tree->free_fun!=NULL)
	{
		...
		stack_Append(&stack_op,&tag_tree->tree_root);
		while(stack_Head(stack_op)->array_length>0)
		{
			op=(PRTREE_NODE)stack_Get(stack_op);
			stack_Remove(&stack_op);
			if(prnode_left(op)!=NULL)stack_Append(&stack_op,&prnode_left(op));
			if(prnode_right(op)!=NULL)stack_Append(&stack_op,&prnode_right(op));
