...
PROXY_CONFIG* proxy_Init(IO_CONFIG *io_config,CHAR_ const *arg)
{
	...
	if((config->regex_list=array_Create(sizeof(PROXY_REGEX)))==NULL)goto fail_return;
	xml_Open(&doc,fp,STORE_ALL);
	
	config_root=xml_Nodebyname(ENCODE_("proxy"),doc.root);
	
	xml_Storedata(&cache,xml_Nodebyname(ENCODE_("buf_size"),config_root),&doc);
	config->buf_size=STRTOUL_(cache,NULL,0);

	node=xml_Nodebyname(arg,config_root);
	for(i=0;i<xml_Nodechilds(node);++i)
	{
		...
		ERROR_OUT_(stderr,ENCODE_("ADD A PORT:%lu\n"),STRTOUL_(cache,NULL,0));
	...
	dlist_Create(&config->owner_list,sizeof(PROXY_OWNER));
