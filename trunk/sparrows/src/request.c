...
HEAD_SHARE* head_Init(size_t buf_size,C_ARRAY MOD_T *mod_table)
{
	...
	hash_Create_Ex(&share->fd_list,&head_Tinyhash,&head_Ensure,sizeof(HEAD_FD),(UINT_)0x0000ffff,&head_Free);
...
int head_Work(HEAD_SHARE *share,HTTP_CONNECT *connect)
{
	...
	ERROR_OUT_(stderr,ENCODE_("HEAD WORK GOT THE FD IS %d\n"),connect->fd);
	...
	else if(len<share->buf_size)
	{
		...
		array_Drop(&head->buf);
		ERROR_OUT_(stderr,ENCODE_("KILL HEAD:%d,THE CONNECT FD IS %d\n"),head->fd,connect->fd);
		i=connect->fd;
		hash_Remove(&share->fd_list,head);
		ERROR_OUT_(stderr,ENCODE_("KILL DONE,THE CONNECT FD IS:%d\n"),connect->fd);
		connect->fd=i;
		...
		for(i=0;i<array_Length(share->mod_table);++i)
		{
			...
			else
			{
				...
				if((state&SELECT_INPUT_)||(state&SELECT_OUTPUT_))
				{
					...
					ERROR_OUT_(stderr,ENCODE_("HEAD WORK IS DONE,RETURN THE FD IS:%d\n"),connect->fd);
					return return_state;
				};
			};
		};
