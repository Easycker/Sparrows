		...
		if(wait_fd>0)
		{
			for(i=0;i<wait_fd;++i)
			{
				...
				if(current_connect!=NULL)
				{
					...
					{
						ERROR_OUT_(stderr,ENCODE_("A NEW CONNECT GOINT TO BE CREATE\n"));
...
int main(int argc,char *argv[])
{
	...
	hash_Create(&connect_list,&connect_Tinyhash,&connect_Ensure,sizeof(HTTP_CONNECT),(UINT_)HASH_SPACE_);

	/*add the listen port to list*/
	event_Add(&connect_list,epoll_fd,listenfd,EPOLLIN|EPOLLET,NULL);
