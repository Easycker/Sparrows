...
#ifndef WITHOUT_WIDECHAR_SUPPORT_
		if(inet_aton(string_Widetoansi(&cache_ansi,cache),&config->addr.sin_addr)<0)goto fail_return;
#else
		if(inet_aton(cache),&config->addr.sin_addr<0)goto fail_return;
#endif
...
#else
		if(regcomp(&host.preg,cache,0)<0)goto fail_return;
#endif
...
#else
			share_mod=dlopen(cache,RTLD_LAZY);
#endif
